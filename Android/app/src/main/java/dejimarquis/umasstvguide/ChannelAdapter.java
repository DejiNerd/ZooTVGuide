        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            if(charSequence!=null && charSequence.length()>0){
                ...
                for(int i = 0; i<filter.size(); i++){
                    if(filter.get(i).channelName.toLowerCase().contains(charSequence)){
                        filtered.add(new Channel(filter.get(i).channelNo,filter.get(i).channelName,filter.get(i).image,i));
                    }
                }
